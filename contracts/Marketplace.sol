...
contract Marketplace is ERC1155, Ownable, ERC1155Burnable, ERC1155Supply {
    ...
    struct tokenDatas {
        ...
        bool nftFlag;
        bool isListed;
        mapping(address => mapping(uint256 => uint256)) buyOrder;
        mapping(address => mapping(uint256 => uint256)) sellOrder;
        mapping(uint256 => address) idOwner;
    }

    mapping(string => collectionDetails) public collectionDetail;
    mapping(string => mapping(uint256 => tokenDatas)) public tokenData;
    ...
    function listItem(
        ...
        require(
            msg.sender == tokenData[collectionName][id].idOwner[id],
            "you are not owner"
        );
    ...
    function creatItem(
        ...
        require(
            tokenData[collectionName][id].nftFlag == false,
            "This is NFT so you can't mint it again."
        );
        if (amount == 1) {
